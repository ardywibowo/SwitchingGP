...
[log_theta, deriv_range] = init_sepjointmtgp_default(signals_train_in, cov_func, num_features, rank_approx);
[log_theta, nl] = learn_sepjointmtgp(log_theta, deriv_range, data, num_iter);
...
for j = 1 : num_contexts
	...
	for i = 1 : length(time)
		...
		length_Lf = num_contexts * rank_approx * (2*num_features - rank_approx +1) / 2;
		length_Lf_curr = length_Lf / num_contexts;
		
		length_baseline = eval(feval(cov_func{1}));
		length_contexts = eval(feval(cov_func{2}));
		
		theta_lf = log_theta((j-1)*length_Lf_curr + 1 : j*length_Lf_curr + 1);
