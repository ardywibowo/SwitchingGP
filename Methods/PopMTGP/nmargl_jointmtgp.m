function [nl, gradnl] = nmargl_jointmtgp(logtheta, logtheta_all, cov_func, time, signals,...
...
length_Lf = rank_approx * (2*num_features - rank_approx +1) / 2; % number of parameters for Lf

% Get Parameters
theta_Lf = logtheta_all(1 : length_Lf);
Lf = vec2lowtri_inchol(theta_Lf, num_features, rank_approx);
Kf = Lf*Lf';
		...
		if (z <= length_Lf) % Gradient wrt  Kf
			for j = 1 : num_contexts
				...
				for i = 1 : num_realizations					
					...
					[o, p] = pos2ind_tri_inchol(z, num_features, rank_approx); % determines row and column
