function [D1, D2] = ncca_spectral( X1, X2, K, scale )
%NCCA_SPECTRAL Non-Gaussian canonical correlation analysis; the estimation 
% is based on two eigen decomposition ("spectral" algorithm) of 
% generalized covariance matrices
%
% [D1, D2] = ncca_spectral( X1, X2, K, scale )
%
% Input: 
...
%
% The model and algorithm (see supplementary material) are proposed by:
...
% Copyright 2016, Anastasia Podosinnikova
  ...
  % Construct one target matrix based on generalized covariance matrices
  v = rand( K, 1 );
  t1 = scale * (W1'*v); 
  t2 = scale * (W2'*v);
  ...
  Q = xx; % Q may (and most probably will) have complex values
  
  D1 = normalize_D_n(pinv(W1)*Q);  D1 = real( D1 );
  D2 = normalize_D_n(pinv(W2)/Q'); D2 = real( D2 );
