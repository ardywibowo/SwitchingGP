...
%
% Copyright (c) 2004, 2005, 2006 by Carl Edward Rasmussen, 2006-05-10.
...
% The erfint function approximates "\int logistic(x) N(x; mu, var) dx" by the
% expression 1/2 + \sum_{i=1}^5 (c_i/2) \int erf(lambda_i x) N(x; mu, var) dx
% = 1/2+\sum_{i=1}^5 (c_i/2) erf(mu/v_i), where  v_i^2 = 2*var+(1/lambda_i)^2,
% and the inputs mu and var are column vectors.

function pout = erfint(mu, var)

ilam2 = [0.44 0.41 0.40 0.39 0.36].^(-2);

c = [ 1.146480988574439e+02
     -1.508871030070582e+03
      2.676085036831241e+03
     -1.356294962039222e+03
      7.543285642111850e+01 ];               % see sigapprox.m for these values
